...
const ProductCard = memo(function ProductCard({
    ...
    const isRetailTier = activeTier?.id === sortedPrices[0]?.id;
    const effectiveUnitPrice = discountPct > 0 && (offerScope === 'all' || isRetailTier)
        ? Math.round(unitPrice * (1 - discountPct / 100) * 100) / 100
        : unitPrice;
    const savingsPerUnit = Math.max(0, baseUnitPrice - effectiveUnitPrice);
    ...
    const hasDiscount = effectiveUnitPrice < baseUnitPrice;
            ...
            <div className="flex justify-between items-start gap-3">
                ...
                {sortedPrices.length > 0 && (
                    <div className="text-right flex-shrink-0">
                        {hasDiscount && (
                            <p className="text-[#adaaaa] text-sm line-through leading-tight">
                                ${baseUnitPrice.toLocaleString('es-AR')} ARS
                            </p>
                        )}
